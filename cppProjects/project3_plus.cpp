...
5. Perform Mulliken Population Analysis and calculate dipole moment based on SCF results
*/
#include <vector>
#include <map>
#include <fstream>
...
public:
    ...
    vector<double> read_rep_int(const char *Workdir, const char *Basis) // µνλσ ≡ µν(µν+1)/2+λσ.
    {
        ...
        string basis = Basis;
        vector<double> rep;
        rep.resize(index_map(index_map(N_AO, N_AO), index_map(N_AO, N_AO)) + 10);
        ifstream fin;
        ...
        for (int i = 0; i < N_AO; i++)
        {
            for (int j = 0; j < N_AO; j++)
            {
                for (int miu = 0; miu < N_AO; miu++)
                {
                    ...
                    {
                        mol_Fock(i, j) += C(j, miu) * C(i, niu) * Fock(miu, niu);
                    }
                }
            }
        }
...
int main(int argc, char *argv[]) // command: ./project3 {-molecule} {-basis set} -{SCF}; default parameter is: h2o STO-3G tight (diis)
{
    ...
    string Workdir = "F:/cpp/ProgrammingProjects-master/Project#03/input/ch4/";
    string output = "F:/cpp/ProgrammingProjects-master/Project#03/input/ch4.log";
    string basis = "STO-3G";
    string inter = "ch4";
    ...
    print_matrix(S, fout, "Overlap matrix.");
    // preparation --> build init-guess Density matrix
    Eigen::MatrixXd S_sym = mol.ortho_overlap(S);
    print_matrix(S_sym, fout, "Symmed Overlap matrix."); // WRONG!
    Eigen::MatrixXd D_init = mol.initial_guess_D(S_sym, H_core);
    print_matrix(D_init, fout, "Inital Density matrix."); // WRONG!
    ...
    print_matrix(mol_Fock, fout, "Fock matrix in molecular basis.");
    ...
    print_matrix(2 * D * S, fout, "P_matrix");
